...
An agent that has no pane here is still reachable without herdr: run it in a scratch terminal under
`script`, replay the capture through a headless VT emulator and dump each row with its own SGR - the
form `herdr pane read <pane> --format ansi` hands over. That is how the `pi` reader's marks
were taken ([ADR 0025](docs/adr/0025-a-pi-pane-reads-by-the-owners-box.md)), and a stub provider
registered through the agent's own extension API drives a real conversation with no API key.
